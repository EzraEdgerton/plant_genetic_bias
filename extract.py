...
for attr,value in thing.iteritems():
...
print thing
"""text = 'EHEHEH THE LIE = ..OF THE LOSERS,\n dddd'
text2 = 'EHEHEH THE LIE = .OF THE LOSERS,\n dddd'
def inner_extractor(start, sub_text):
				...
				adder = 3
				print sub_text[startindex + 2]
				print sub_text[startindex + adder]
				print sub_text[startindex + 4]
				if sub_text[startindex + 3] == '.':
					adder = 4
				print adder
				endindex = text.find(',\n', start)
				print text[startindex + 4 : endindex]
				#return [text[startindex + adder : endindex], endindex]

#val = text.find('= {')
#print text[val + 4:]
inner_extractor(0, text2)
text2 = text2.replace('E', 'u')
print text2"""
"""filename = sys.argv[1]
	...
	with open('genetic_tools.csv', 'rb') as genetic_terms:
		with open('countries.csv', 'rb') as countries:
			...
			def inner_extractor(start):
				...
				print text[startindex + 2 : endindex]
			...
			def extractor(textindex):
				...
				if title_text != '':
					data.append({ 'focalspecies' : focal_species, 'focalspeciesCOO' : countries, 'year' : year_text, 'journal' : journal_text, 'genetictool' : genetic_tool, 'authorsCOO' : authors_coo, 'fundingorganization' : funding_text})# \n {} \n {} \n {} \n {} \n {} \n {} \n \n'.format(focal_species, countries, year_text, journal_text, genetic_tool, authors_coo, funding_text))
